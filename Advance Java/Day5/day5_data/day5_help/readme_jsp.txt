...
Typically used -- for including static contents (can be used to include dynamic contents)
eg ---one.jsp
...
3. Auto. translation between  (string based) req. params & (primitive)JB props(string--->primitive data types automatically done by WC)

What is a java bean ?
Its a server side attribute , created by WC , added under desired scope(page|request|session|application)
Details 
...
1.Copy above JAR into run-time classpath(copy jars either in <tomcat_home>/lib OR <WEB-INF>/lib
2. Use taglib directive to include JSTL tag library into JSP.
tag=action
...
eg --- To import JSTL supplied core tag lib
...
3.1 JSTL tag for send redirect(clnt pull II)
<c:redirect url="nextPageURL"/>
eg : In one.jsp
<c:redirect url="two.jsp"/>
WC invokes ---response.sendRedirect("two.jsp");
+ URL rewriting


3.2 JSTL tag for iteration
<c:forEach var="loop local var" items="iterable prop">
Body of for-each
</c:forEach>

eg : <c:forEach var="cat" items="${sessionScope.book.listCategories()}">
${cat}
</c:forEach>

WC invokes --for(String cat : session.getAttribute("book").listCategories())
 out.print(cat)
